...
} from "./szdiagnostic/szdiagnostic_web_pb";
...
export class SzGrpcWebDiagnostic extends SzGrpcWebBase implements SzDiagnostic {
  ...
  checkRepositoryPerformance(
    secondsToRun: number,
  ): Promise<CHECK_DATASTORE_PERFORMANCE_RESPONSE | SzError> {
    return new Promise((resolve, reject) => {
      ...
      this.waitForReady(this.getDeadlineFromNow(), (err) => {
        ...
        this.client.checkRepositoryPerformance(
          request,
          this._metadata,
          (err, res: CheckRepositoryPerformanceResponse) => {
            if (err) {
            ...
            let result = JSON.parse(
              res.getResult(),
            ) as CHECK_DATASTORE_PERFORMANCE_RESPONSE;
  ...
  getRepositoryInfo(): Promise<string> {
    return new Promise((resolve, reject) => {
      ...
      this.waitForReady(this.getDeadlineFromNow(), (err) => {
        ...
        this.client.getRepositoryInfo(
          request,
          this._metadata,
          (err, res: GetRepositoryInfoResponse) => {
            if (err) {
              let _err = newException(err);
              reject(_err);
              throw _err;
  ...
  getFeature(featureId: number): Promise<string> {
    return new Promise((resolve, reject) => {
      ...
      this.waitForReady(this.getDeadlineFromNow(), (err) => {
        ...
        this.client.getFeature(
          request,
          this._metadata,
          (err, res: GetFeatureResponse) => {
            if (err) {
              let _err = newException(err);
              reject(_err);
              throw _err;
  ...
  purgeRepository(): Promise<undefined> {
    return new Promise((resolve, reject) => {
      ...
      this.waitForReady(this.getDeadlineFromNow(), (err) => {
        ...
        this.client.purgeRepository(
          request,
          this._metadata,
          (err, res: PurgeRepositoryResponse) => {
            if (err) {
              let _err = newException(err);
              reject(_err);
              throw _err;
