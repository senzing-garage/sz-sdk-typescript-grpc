...
export class SzGrpcWebEnvironment extends EventTarget {
  ...
  public getConfigManager() {
    if (!this._configClient) {
      ...
      if (!this._configClient)
        this._configClient = new SzConfigClient(
    ...
    if (!this._configManager || !this._configManagerClient) {
      ...
      if (!this._configManagerClient)
        this._configManagerClient = new SzConfigManagerClient(
          this.connectionString,
          this.credentials,
          this._grpcOptions,
        );
      ...
      if (!this._configManager)
        this._configManager = new SzGrpcWebConfigManager({
          client: this._configManagerClient,
          configClient: this._configClient,
          grpcOptions: this._grpcOptions,
  ...
  public getDiagnostic() {
    if (!this._diagnostic || !this._diagnosticClient) {
      ...
      if (!this._diagnosticClient)
        this._diagnosticClient = new SzDiagnosticClient(
          this.connectionString,
          this.credentials,
          this._grpcOptions,
        );
      ...
      if (!this._diagnostic)
        this._diagnostic = new SzGrpcWebDiagnostic({
          client: this._diagnosticClient,
          grpcOptions: this._grpcOptions,
        });
    }
  ...
  public getEngine() {
    if (!this._engine || !this._engineClient) {
      ...
      if (!this._engineClient)
        this._engineClient = new SzEngineClient(
          this.connectionString,
          this.credentials,
          this._grpcOptions,
        );
      ...
      if (!this._engine)
        this._engine = new SzGrpcWebEngine({
          client: this._engineClient,
          grpcOptions: this._grpcOptions,
        });
    }
    return this._engine;
  ...
  public getProduct() {
    if (!this._product || !this._productClient) {
      ...
      if (!this._productClient)
        this._productClient = new SzProductClient(
          this.connectionString,
          this.credentials,
          this._grpcOptions,
        );
      ...
      if (!this._product)
        this._product = new SzGrpcWebProduct({
          client: this._productClient,
          grpcOptions: this._grpcOptions,
        });
    }
  ...
  constructor(parameters: SzGrpcWebEnvironmentOptions) {
    ...
    // in getters
    const { connectionString, credentials, grpcOptions, isTestEnvironment } =
      parameters;
