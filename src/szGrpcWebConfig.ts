...
export class SzGrpcWebConfig extends SzGrpcWebBase implements SzConfig {
  ...
  registerDataSource(dataSourceCode: string) {
    return new Promise<string>((resolve, reject) => {
      ...
      this.waitForReady(this.getDeadlineFromNow(), (err) => {
        ...
        this.client.registerDataSource(
          request,
          this._metadata,
          (err, res: RegisterDataSourceResponse) => {
            if (err) {
  ...
  registerDataSources(dataSourceCodes: string[]) {
    ...
    let addDataSourcesSequentially = (
      dataSources: string[],
      callback?: Function,
    ) => {
      ...
      let next = () => {
        ...
        this.registerDataSource(dsName as string).then((resp) => {
    ...
    return new Promise<string[]>((resolve, reject) => {
      let addDataSources = addDataSourcesSequentially(
        dataSourceCodes,
        (results: string[], error?: Error | undefined) => {
          if (error) {
            reject(error);
          }
          resolve(results);
  ...
  unregisterDataSource(dataSourceCode: string) {
    return new Promise<boolean>((resolve, reject) => {
      ...
      this.waitForReady(this.getDeadlineFromNow(), (err) => {
        ...
        this.client.unregisterDataSource(
          request,
          this._metadata,
          (err, res: UnregisterDataSourceResponse) => {
            if (err) {
              reject(newException(err));
              return;
            }
  ...
  unregisterDataSources(dataSourceCodes: string[]) {
     ...
     */
    /** private class used for managing sequential requests */
    let removeDataSourcesSequentially = (
      dataSources: string[],
      callback?: Function,
    ) => {
      let _dataSources = dataSources;
      ...
      let next = () => {
        ...
        //return new Promise((resolve, reject) => {
        this.unregisterDataSource(dsCode as string).then((resp) => {
          // add response to results
      ...
      // set up onComplete promise now that we know the number of requests
      _onComplete = new Promise<{ DSRC_CODE: string; DELETED: boolean }[]>(
        (resolve, reject) => {
          if (_responses.length >= _dataSources.length) {
            resolve(_responses);
          }
    ...
    return new Promise<{ DSRC_CODE: string; DELETED: boolean }[]>(
      (resolve, reject) => {
        let removeDataSources = removeDataSourcesSequentially(
          dataSourceCodes,
          (
            results: { DSRC_CODE: string; DELETED: boolean }[],
  ...
  getDataSourceRegistry(): Promise<{ DSRC_ID: number; DSRC_CODE: string }[]> {
    return new Promise((resolve, reject) => {
      ...
      this.waitForReady(this.getDeadlineFromNow(), (err) => {
        ...
        request.setConfigDefinition(this.definition);
        this.client.getDataSourceRegistry(
          request,
          this._metadata,
          (err, res: GetDataSourceRegistryResponse) => {
            if (err) {
              reject(newException(err));
              return;
            }
  ...
  verifyConfig(configDefinition?: string) {
    return new Promise((resolve, reject) => {
      ...
      this.waitForReady(this.getDeadlineFromNow(), (err) => {
        ...
        request.setConfigDefinition(
          configDefinition ? configDefinition : this.definition,
        );
        this.client.verifyConfig(
          request,
          this._metadata,
          (err, res: VerifyConfigResponse) => {
            if (err) {
              reject(newException(err));
              return;
            }
